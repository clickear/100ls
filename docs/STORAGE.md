...
配置完成后，建议运行以下脚本来验证存储服务是否连通（特别是七牛云模式）：
...
```bash
# 确保已安装 tsx
npx tsx scripts/debug/test-qiniu.ts
```

**预期结果：**
1.  控制台显示 `🚀 Starting Qiniu Upload Test...`。
2.  成功后显示 `✅ Upload Successful!` 并输出一个 `🔗 Public URL`。
3.  您可以点击该链接，如果在浏览器中能看到测试文本，说明配置完全正确。

### 验证本地存储
本地存储通常不需要专门测试，只要 `backend/media` 目录存在且具有写权限即可。您可以尝试导入一个视频，观察 `backend/media` 是否生成了对应的文件夹。
